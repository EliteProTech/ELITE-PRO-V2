...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    EliteProTech.rememberMessage = (message) => {
        const key = message?.key;
        if (!key?.id || !message?.message) return;
        const cacheKey = `${key.remoteJid || key.remoteJidAlt || ''}:${key.participant || key.participantAlt || ''}:${key.id}`;
        messageCache.delete(cacheKey);
        messageCache.set(cacheKey, message.message);
        while (messageCache.size > MESSAGE_CACHE_LIMIT) {
            messageCache.delete(messageCache.keys().next().value);
        }
    };

    EliteProTech.getStoredMessage = (key) => {
        if (!key?.id) return null;
        const chat = key.remoteJid || key.remoteJidAlt || '';
        const participant = key.participant || key.participantAlt || '';
        return messageCache.get(`${chat}:${participant}:${key.id}`) ||
            messageCache.get(`${chat}::${key.id}`) ||
            [...messageCache.entries()].find(([cacheKey]) => cacheKey.endsWith(`:${key.id}`))?.[1] ||
            null;
    };

    const sendMessage = EliteProTech.sendMessage.bind(EliteProTech);
    EliteProTech.sendMessage = async (...args) => {
        const sent = await sendMessage(...args);
        EliteProTech.rememberMessage(sent);
        return sent;
    };
        ...
        if (quotedRaw) {
            ...
            if (!realQuoted) {
                ...
                if (!type || !quotedContent) {
                    ...
                    quotedObj.rawMessage = quotedRaw;
