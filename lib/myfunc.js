...
export async function smsg(EliteProTech, m) {
    ...
    if (m.key) {
        ...
        m.isDM = !m.isGroup;
        m.sender = EliteProTech.decodeJid(m.fromMe && EliteProTech.user.id || m.key.participant || m.chat || '');
        ...
        if (m.sender.endsWith('@lid')) {
            if (m.isGroup) {
                ...
                m.sender = p?.phoneNumber || await EliteProTech.resolveLidToJid(m.sender);
            } else {
                m.sender = await EliteProTech.resolveLidToJid(m.sender);
            }
        }

        if (m.chat.endsWith('@lid') && !m.isGroup) {
            const originalChat = m.chat;
            m.chat = await EliteProTech.resolveLidToJid(m.chat);
        }
            ...
            if (!realQuoted) {
                ...
                if (!quotedContent) {
                    ...
                    if (quotedObj.sender && quotedObj.sender.endsWith('@lid')) {
                        if (m.isGroup) {
                            let meta = await getGroupMetadata(EliteProTech, m.chat);
                            const p = meta?.participants?.find(u => u.id === quotedObj.sender);
                            quotedObj.sender = p?.phoneNumber || await EliteProTech.resolveLidToJid(quotedObj.sender);
                        } else {
                            quotedObj.sender = await EliteProTech.resolveLidToJid(quotedObj.sender);
                        }
                    }

                    if (quotedObj.chat && quotedObj.chat.endsWith('@lid') && !m.isGroup) {
                        const originalQuotedChat = quotedObj.chat;
                        quotedObj.chat = await EliteProTech.resolveLidToJid(quotedObj.chat);
                    }
