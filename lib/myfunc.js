...
export async function smsg(EliteProTech, m) {
    ...
    if (m.message) {
        // WhatsApp commonly wraps a chat's first incoming message in an
        // ephemeral/device wrapper.  The reply context belongs to the inner
        // message, so read the real content before looking for contextInfo.
        const messageContent = extractRealMessage(m.message);
        m.mtype = Object.keys(messageContent)[0];
        ...
        const contexts = [
            m.msg?.contextInfo,
            m.msg?.messageContextInfo,
            messageContent?.contextInfo,
            messageContent?.messageContextInfo,
            m.message?.contextInfo,
            m.message?.messageContextInfo
        ].filter(Boolean);
        const quoteContext = contexts.find(context => context.quotedMessage);
        let quotedRaw = quoteContext?.quotedMessage || null;

        // This is deliberately small and contains no message text. It makes it
        // possible to diagnose a client that sends a reply without quote data:
        // evaluate `m.quoteDebug` after receiving a null quoted value.
        m.quoteDebug = {
            mtype: m.mtype,
