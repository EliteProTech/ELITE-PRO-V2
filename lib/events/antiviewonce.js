...
let handler = async (EliteProTech, { messages, type }) => {
    ...
    for (const raw of messages || []) {
        ...
        const groupRule = isGroup ? global.antiViewOnceGroups?.[chat] : null
        const destinationMode = isGroup
            ? (groupRule === 'off' ? null : groupRule || (global.antiViewOnceGlobal ? 'dm' : null))
            : (global.antiViewOnceGlobal ? 'dm' : null)
        ...
        try {
            ...
            const destination = destinationMode === 'chat' ? chat : EliteProTech.decodeJid(EliteProTech.user.id)
