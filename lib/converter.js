...
async function animatedWebpToVideo(buffer) {
    ...
    try {
        ...
        if (!img.hasAnim || !img.frames || img.frames.length === 0) {
            throw new Error('Not an animated webp (no frames found)')
        }
        ...
        for (let i = 0; i < frameCount; i++) {
            ...
            const rgbaPath = path.join(frameDir, `frame_${String(i).padStart(4, '0')}.rgba`)
            const framePath = path.join(frameDir, `frame_${String(i).padStart(4, '0')}.png`)
            
            fs.writeFileSync(rgbaPath, frameData)
            ...
            framePaths.push({ path: framePath })
        }

        let listContent = ''
        for (let i = 0; i < frameCount; i++) {
            const delayMs = img.frames[i]?.delay ?? 100
            const duration = Math.max(delayMs / 1000, 0.02)
            listContent += `file '${framePaths[i].path.replace(/'/g, "'\\''")}'\nduration ${duration}\n`
        }
