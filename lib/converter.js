...
async function animatedWebpToVideo(buffer) {
    ...
    try {
        ...
        await runFfmpeg([
            ...
            '-framerate', '15',
            '-i', rawPath,
            '-filter_complex', `color=c=white:s=${image.width}x${image.height}:r=15[background];[background][0:v]overlay=shortest=1,scale=trunc(iw/2)*2:trunc(ih/2)*2`,
            '-c:v', 'libx264',
            '-preset', 'veryfast',
            '-crf', '18',
            '-pix_fmt', 'yuv420p',
...
export async function toVideo(buffer, inputExt = 'bin') {
    if (inputExt === 'webp') {
        ...
        const image = new webp.Image()
        await image.load(buffer)
        if (image.hasAnim && image.frames?.length) return animatedWebpToVideo(buffer)
    }
    const inputPath = await writeInput(buffer, inputExt)
    const outputPath = tmpFile('mp4')
    try {
        const isStillImage = ['webp', 'jpg', 'jpeg', 'png'].includes(inputExt)
        await runFfmpeg([
            ...(isStillImage ? ['-loop', '1'] : []),
            '-i', inputPath,
            ...(isStillImage ? ['-t', '3'] : []),
